...
bon=Blueprint("bon", __name__)

API_CONTRAT = "https://back-mcs-v1.herokuapp.com/web/contrat?id=90046097"
URL_FACTURE="https://back-mcs-v1.herokuapp.com/web/facture?id=64"
URL_ADRESSE="https://back-mcs-v1.herokuapp.com/web/client?id=63"

@bon.route('/pdf',methods = ['POST', 'GET'])
def contrat_pdf():
    ...
    facture_adresse['adresses'][2]['STREET_NUMBER']+ " " +facture_adresse['adresses'][2]['ROUTE']+'\n'+
    str(facture_adresse['adresses'][2]['codepostal'])+ " " +facture_adresse['adresses'][2]['ville']+'\n\n'
    "Demandé par : "+str(facture_adresse['contactes'][1]['civilite'])+ " " +facture_adresse['contactes'][1]['prenom']+ " " +facture_adresse['contactes'][1]['nom']+'\n'+
    "Tel : "+str(facture_adresse['contactes'][1]['telmobile'])+'\n'+
    "Fax : " ,border=1)
    pdf.ln(7)
    tmpVarX = pdf.get_x()
    tmpVarY = pdf.get_y()
    ...
    for i in range(len(contrat_data['equipements'])):
        if facture['type_remise']=="e":
          montant_net=float(contrat_data['equipements'][i]['prix'])-float(facture['remise'])
        else: 
          montant_net=float(contrat_data['equipements'][i]['prix'])*float(facture['remise'])/100
        ...
        pdf.set_font('Arial','B',10) 
        pdf.cell(  epw/10, 2*th, txt=str(contrat_data['equipements'][i]['prix']), align='C', border=1)
        pdf.cell(  epw/20, 2*th, txt=str(contrat_data['equipements'][i]['remise']), align='C', border=1)
        pdf.cell(  epw/11, 2*th, txt=str(montant_net), align='C', border=1)
        montantHT=montantHT+montant_net
    ...
    pdf.cell(30, 2*th, str(montantHT) ,border=1)
    pdf.ln(2*th) 
    pdf.cell(tmpVarX+120)
    ...
    pdf.cell(30, 2*th, str(contrat_data['total']) ,border=1)
    pdf.ln(10)
    #mentions----------------------------------
